...
def generate_launch_description():
    ...
    #3.完成自定义可执行文件的动作，并且该节点调用顺序有要求（要在掉头完毕才能执行）
    exer01 = Node(package = "cpp07_exercise", executable = "exer01_pub_sub")
    #怎么控制节点的执行顺序？需要通过注册事件完成
    #创建时间注册对象，在对象中声明针对哪个目标节点，在哪个时间触发时，执行哪种操作
    register_rotate_exit_event = RegisterEventHandler( #事件注册器（动作）
        #创建一个事件监听器
        event_handler = OnProcessExit( 
            target_action = rotate, #监听的目标动作
            on_exit = exer01 #事件触发后要执行的动作
        )
    )

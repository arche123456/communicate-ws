/*
  ...
  流程：
    ...
    2.初始化ROS客户端
    3.自定义节点类
      3.1创建订阅方，订阅原生乌龟坐标位姿（/turtle1/pose）
      3.2创建服务端
      3.3回调函数需要解析客户端数据，并响应结果到客户端
...
#include "turtlesim/msg/pose.hpp"       //原生乌龟的位姿信息
...
class Exer02Server: public rclcpp::Node
{
  public:
    ...
    {
        RCLCPP_INFO(this->get_logger(),"案例2对象创建");
        //3.1创建订阅方，订阅原生乌龟坐标位姿（/turtle1/pose）
  ...
  private:
    ...
    void distance_cb(Distance::Request::SharedPtr request, Distance::Response::SharedPtr response)
    {
        //1.解析出目标点坐标
...
int main(int argc, char ** argv)
{
  ...
  //4.调用spin函数，传入自定义类对象指针
  rclcpp::spin(std::make_shared<Exer02Server>());//持续处理传入节点的所有回调
  //5，释放资源
  rclcpp::shutdown();
  return 0;
}
